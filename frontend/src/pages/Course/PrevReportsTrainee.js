...
const PrevReportsTrainee = () => {
  ...
  const handleSubmit = async(event) => {
     ...
     try {
       const res = await axios.patch("/individualtrainee/addFollowUp", follow, config);
       Swal.fire({
         position: "top-end",
         icon: "success",
    ...
    {reports.map((report, index) => {
        return(
          <Accordion.Item eventKey={index}>
             ...
             <Accordion.Header ><div style={{color: report["status"] === 'pending' ? '#A00407' : '#28D770' }}>Problem {index+1} : {report["title"]}</div></Accordion.Header>
    
    
              
              
             <Accordion.Body>
              <label style={{color: report["status"] === 'pending' ? '#A00407' : '#28D770' }}>You problem status : {report["status"]}</label>
              <br></br>
              <label style={{color:"#A00407"}}>You problem description :</label><br></br>
                {report["description"]}
                ...
                ? <span></span>
                    ...
                    <>
                     ...
                     <div>
                     <Accordion.Header>Your Follow Ups</Accordion.Header>
                    <br></br>
                    <Accordion.Body>
          ...
          {report["status"]==="pending"
            ...
            <div>
            <Popup trigger={<Button >Add Follow Up</Button>} position="right center">
        <div>
