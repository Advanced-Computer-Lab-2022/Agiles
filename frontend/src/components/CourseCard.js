...
export const CourseCard = (props) => {
  ...
  return (
    <div className="card">
      ...
      {props.data.price ===0?(<div>Price : Free</div>):(<>{!window.sessionStorage.getItem("factor")?( <div>Price: {(props.data.price)} USD</div>):( <div>Price: {(props.data.price)*(window.sessionStorage.getItem("factor"))} {window.sessionStorage.getItem("currency").toUpperCase()}</div>)}  
</>)}
      <button onClick={handleClick}>View Course</button>
    </div>
  );
